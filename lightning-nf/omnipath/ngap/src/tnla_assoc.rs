...
impl TnlaAssociation {
	pub fn new(stream: SctpStream) -> Result<Self, NetworkError> {
		// Get local address
		let local_addr = stream
			.local_addr()
			.map_err(NetworkError::LocalAddressError)?;

		// Get remote address
		let remote_addr = stream
			.peer_addr()
			.map_err(NetworkError::RemoteAddressError)?;
	...
	pub async fn read_data(&self) -> Result<BytesMut, NetworkError> {
		...
		self.stream
			...
			.await
			.map_err(NetworkError::ReadError)?;
		let data = buf.freeze();
		Ok(data)
	}
